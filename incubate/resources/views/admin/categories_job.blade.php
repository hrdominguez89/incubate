...
<div class="content-area">
   <div class="breadcrumb-bar">
      <div class="page-title">
         <h1><i class="ti-bookmark-alt"></i> Categorías</h1>
      </div>
...
<script type="text/javascript">
  ...
  app.controller('species', function($scope, $http, $window) {
    ...
    $scope.btn = "Guardar";
    $scope.rows = function() {
      $http.get('{{url("categories_job_lists")}}').then(function successCallback(response)   {
    ...
    $scope.borrar = function(id) {
      ...
      function() {
            // Url 
            var route = "{{url('/')}}/job-categories/" + id + "";
            var token = $("#token").val();
            //Enviar Datos
            $.ajax({
              url: route,
  ...
  $('#navsm').sortable({
        ...
        update: function() {
            ...
            $.post("{{url('move-categories-job')}}", order, function(theResponse) {
