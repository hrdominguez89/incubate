                  ...
                  <div class="widget z-depth-1">
                     ...
                     <div class="widget-content">
                      <div class="input-field">
                        <label>Cantidad<small style="color: #fd2923">*</small></label>
...
<script type="text/javascript">
  ...
  app.controller('species', function($scope, $http, $window) {
    $scope.titulo = "Nueva cantidad";
    $scope.btn = "Guardar";
    $scope.rows = function() {
      $http.get('{{url("members_lists")}}').then(function successCallback(response)   {
    ...
    $scope.guardar = function() {
      ...
      var token = $("#token").val();
      if (name == "") {
        $.growl.error({ title: "<i class='fa fa-exclamation-circle'></i> Atención", message: "Ingresá la cantidad"});
        $('#name').focus();
        return (false);
      }
      else {
    ...
    $scope.asignarValor = function(id,name) {
      $("#id").val(id);
      $("#name").val(name);
      $scope.titulo = "Actualizá tu cantidad";
      $scope.btn = "Guardar";
    }
  });
  $('#navsm').sortable({
        ...
        update: function() {
            ...
            $.post("{{url('move-members')}}", order, function(theResponse) {
