...
<div class="content-area">
   <div class="breadcrumb-bar">
      <div class="page-title">
         <h1><i class="ti-gallery"></i> Categorías</h1>
      </div>
...
<script type="text/javascript">
	...
	app.controller('species', function($scope, $http, $window) {
		...
		$scope.btn = "Guardar";
		$scope.rows = function() {
			$http.get('{{url("categories_post_lists")}}').then(function successCallback(response)   {
      ...
      if (name == "") {
		...
		$scope.borrar = function(id) {
			...
			function() {
            // Url 
            var route = "{{url('/')}}/post-categories/" + id + "";
            var token = $("#token").val();
            //Enviar Datos
            $.ajax({
            	url: route,
	...
	$('#navsm').sortable({
        ...
        update: function() {
            ...
            $.post("{{url('move-categories-post')}}", order, function(theResponse) {
