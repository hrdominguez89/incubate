...
<div class="content-area">
   <div class="breadcrumb-bar">
      <div class="page-title">
         <h1><i class="ti-pencil-alt"></i> Categorías</h1>
      </div>
...
<script type="text/javascript">
	...
	app.controller('species', function($scope, $http, $window) {
		$scope.titulo = "Nueva categoría";
		$scope.btn = "Guardar";
		$scope.rows = function() {
			$http.get('{{url("categories_information_lists")}}').then(function successCallback(response)   {
      ...
      if (name == "") {
        ...
        return (false);
				...
				if (id == "") {
					$.ajax({
						url: "{{url('information-categories')}}",
						headers: {
							'X-CSRF-TOKEN': token
						},
						type: 'POST',
              ...
              $("#name_en").val('');
					...
					$.ajax({
						url: "{{url('/')}}/information-categories/" + id + "",
						headers: {
							'X-CSRF-TOKEN': token
						},
						type: 'PUT',
		...
		$scope.borrar = function(id) {
			...
			function() {
            // Url 
            var route = "{{url('/')}}/information-categories/" + id + "";
            var token = $("#token").val();
            //Enviar Datos
            $.ajax({
            	url: route,
      ...
      $("#name").val(name);
      $("#name_en").val(name_en);
      $scope.titulo = "Actualizá tu categoría";
      $scope.btn = "Guardar";
    }
  });
	$('#navsm').sortable({
        ...
        update: function() {
            ...
            $.post("{{url('move-categories-information')}}", order, function(theResponse) {
