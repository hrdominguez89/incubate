            ...
            <div class="col s12">
               <div class="widget z-depth-1">
                 <div class="widget-bar"> 
                  <div class="col s4">
                     <a class="btn orange" href="{{url('event/create')}}"> <i class="fa fa-plus-circle" aria-hidden="true"></i> Nuevo Evento </a>
                  </div>
                  <div class="col s8">
                     ...
                     <table width="100%" style="width:100%" class="table">
                       <thead>
                           ...
                           <tr ng-repeat="row in filtered = (list | filter:search | orderBy : predicate :reverse) | startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit" id="item_@{{row.id}}">
                              ...
                              <td style="vertical-align: middle; text-align: center;"><div>  @{{row.created_at | date : MM/dd/yyyy }} </div><div>  @{{row.end_date | date : MM/dd/yyyy }} </div></td>
                              <td style="vertical-align: middle;">
                              <span class="controls">
                                  
                                <a href="{{url('/')}}/event/@{{row.id}}/edit" class="edituser blue"><i class="ti-pencil-alt"></i> Editar</a>
...
app.controller('paginado', function($scope, $http, $window) {
    //Listar Paginas 
    $scope.rows = function() {
        $http.get("{{url('events_lists')}}").then(function successCallback(response)  {
    ...
    $scope.borrar = function(id) {
     ...
     $scope.buscar_fecha= function(){
        ...
        if ($('#fecha_final').val() == "") {
          ...
          var url="<?php echo url('/');?>/excel/events/"+fecha_inicial+"/"+fecha_final;
          window.open(url);
        }
      }
});
$('#navsm').sortable({
    ...
    update: function() {
        ...
        $.post("{{url('move-event')}}", order, function(theResponse) {
