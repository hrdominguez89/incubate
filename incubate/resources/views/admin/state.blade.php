...
<div class="content-area">
   <div class="breadcrumb-bar">
      <div class="page-title">
         <h1><i class="ti-bookmark-alt"></i> Estadíos</h1>
      </div>
...
<script type="text/javascript">
  ...
  app.controller('species', function($scope, $http, $window) {
    $scope.titulo = "Nuevo estadío";
    $scope.btn = "Guardar";
    $scope.rows = function() {
      $http.get('{{url("states_lists")}}').then(function successCallback(response)   {
    ...
    $scope.guardar = function() {
      ...
      var token = $("#token").val();
      if (name == "") {
        $.growl.error({ title: "<i class='fa fa-exclamation-circle'></i> Atención", message: "Ingresá el estadío"});
        $('#name').focus();
        return (false);
      }
      else {
    ...
    $scope.asignarValor = function(id,name) {
      $("#id").val(id);
      $("#name").val(name);
      $scope.titulo = "Actualizá tu estadío";
      $scope.btn = "Guardar";
    }
  });
  $('#navsm').sortable({
        ...
        update: function() {
            ...
            $.post("{{url('move-states')}}", order, function(theResponse) {
