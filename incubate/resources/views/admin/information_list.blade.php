            ...
            <div class="col s12">
               <div class="widget z-depth-1">
                 <div class="widget-bar"> 
                  <div class="col s4">
                     <a class="btn orange" href="{{url('information/create')}}"> <i class="fa fa-plus-circle" aria-hidden="true"></i> Nueva Información </a>
                  </div>
                  <div class="col s8">
                         ...
                         <thead>
                           ...
                           <tr ng-repeat="row in filtered = (list | filter:search | orderBy : predicate :reverse) | startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit" id="item_@{{row.id}}">
                              ...
                              <span class="controls">
                                ...
                                <a href="{{url('/')}}/information/@{{row.id}}/edit" class="edituser blue"><i class="ti-pencil-alt"></i> Editar</a>
...
app.controller('paginado', function($scope, $http, $window) {
    //Listar Paginas 
    $scope.rows = function() {
        $http.get("{{url('informations_lists')}}").then(function successCallback(response)  {
    ...
    $scope.borrar = function(id) {
     ...
     $scope.buscar_fecha= function(){
        ...
        if ($('#fecha_final').val() == "") {
          ...
          var url="<?php echo url('/');?>/excel/informations/"+fecha_inicial+"/"+fecha_final;
          window.open(url);
        }
      }
});
$('#navsm').sortable({
    ...
    update: function() {
        ...
        $.post("{{url('move-information')}}", order, function(theResponse) {
